...
def get_arguments() -> int:
    ...
    parser.add_argument("-d", "--dir", help="directory containing .out intermediate files from PSMC", required=True)
...
def load_files() -> int:
    ...
    if (len(psmc_files) == 0):
        msg = f"No *.out files were found in {input_dir}"
        sys.exit(msg)
...
def get_spp_name(text_file: str) -> str:
...
def plot_ne() -> int:
    ...
    for i, psmc_file in enumerate(psmc_files):
        data  = Data()
        color = colors[i] 
        spp   = get_spp_name(psmc_file)
        parse_text(psmc_file, data)
        axe.step(data.time, data.pop_sizes, color=color, alpha=1.0, 
                 linestyle = '-', linewidth = 1, label=spp)
    axe.legend(loc="upper left", bbox_to_anchor=(1, 1), fancybox=True, shadow=True, ncol=1)
